...
<title>Set up for Component</title>
<!-- Favicon-->
<!-- Include jQuery -->
...
<body>
	...
	<a href="${pageContext.request.contextPath}/AddNewVendorLimit.jsp" class="btn btn-success mt-2">Add Agency Limit +</a>
				...
				<table class="table table-bordered table-striped" id="trainingTable"
					...
					<thead class="thead-dark"
						...
						<tr>
							...
							<th>Budgeted Year</th>
							<th>Agency Name </th>
							<th style="display: none">Approved Rate</th>
							<th style="display: none" >Actual Rate</th>
							<th>Unit</th>
							<th>Status</th>
							<th>Action</th>
	...
	$('#trainingTable').on('click', '.truncated-cell', function() {
		...
		function fetchDataAndDisplay() {
			$.ajax({
				type : "GET",
				url : "${pageContext.request.contextPath}/api/vendorslist",
				dataType : "json",
		...
		function displayTrainingData(data) {
			...
			for (var i = 0; i < trainings.length; i++) {
				...
				row.append("<td >" + training.vendorName + "</td>");
 
				row.append("<td style='display: none;'>" + training.approvedrate + "</td>");
				row.append("<td style='display: none;'>" + training.actualrate + "</td>");
		...
		function deleteTraining(button,id) {
			...
			$.ajax({
				type : "Delete",
				url : "${pageContext.request.contextPath}/api/deleteVendorLimit/" + id,
		...
		function editTraining(sl) {
			...
			var editUrl = "${pageContext.request.contextPath}/EditVendorLimit.jsp?sl=" + sl;// Redirect to the EditTraining.jsp page with the query string parameters
