									...
									<div class="mb-2 col-3">
										<label for="resourceType">Resource Type:<span class="required-field">*</span></label>
										 ...
										 <select
											...
											<option value="Mandal Level Trainer">Mandal Level Trainer</option>
...
<!-- 										       <div class="dropdown-symbol">&#9660;</div> -->
									...
									<div class="mb-2 col-3">
										<label for="numberOfDays">Number of Days:<span class="required-field">*</span></label> 
									...
									<div class="mb-2 col-3">
    <label for="attendedyear">Attended year<span class="required-field">*</span></label>
    <select id="attendedyear" name="attendedyear" class="form-control1" required style="font-size: 13px">
	...
	<script>
		$.ajax({
			url : "${pageContext.request.contextPath}/api/previousTrainings",
			//url: "${pageContext.request.contextPath}${pageContext.request.contextPath}/api/search/resource",
			type : "GET",
			dataType : "json",
	...
	<script>
		...
		var treasuryId = sessionStorage.getItem("UserName");		// Check if treasuryId is present in session storagevar
		...
		function showForm1() {
			...
			var treasuryId = sessionStorage.getItem("UserName");
			if (treasuryId) {
				// Use the retrieved treasuryId in the AJAX request
				$.ajax({
					type : "GET",
					url : "${pageContext.request.contextPath}/api/findteacher?treasuryid=" + treasuryId,
					//url: "${pageContext.request.contextPath}${pageContext.request.contextPath}/api/findteacher?treasuryid=" + treasuryId,
					dataType : "json",
...
<script>
   ...
   // var cfmsId = 14030407;
    ...
    function showForm2() {
        ...
        $.ajax({
            type: "GET",
            url: "${pageContext.request.contextPath}/api/privateteacher?cfms_id=" + cfmsId,
            dataType: "json",
            success: function (response) {
...
//          }
		...
		function fun() {
			...
			if (document.getElementById("enableRadioTrani").checked) {
				...
				var formDataOriginal = {
					...
					attendedyear : $("#trainingForm #attendedyear").val(),
				};
				...
				for (var i = 1; i <= clonedForms; i++) {
					var formData2 = {
						...
						attendedyear : $("#trainingForm_" + i + " #attendedyear").val(),
					};
				...
				$
						.ajax({
							type : "POST",
							url : "${pageContext.request.contextPath}/api/save/previoustrainings",
							contentType : "application/json",
							data : JSON.stringify(formDataArray),
							success : function(secondResponse) {
							console.log("Second POST request (${pageContext.request.contextPath}/save/previoustrainings) successful:",secondResponse);
							},
							error : function(xhr, status, error) {
								console .error("Second POST request (${pageContext.request.contextPath}/save/previoustrainings) error:",												error);
		...
		function saveTraining() {
			...
			$.ajax({
				type : "POST",
				url : "${pageContext.request.contextPath}/api/savetrainings",
				contentType : "application/json",
				...
				success : function(response) {
					...
					setTimeout(function() {
						window.location.href = '${pageContext.request.contextPath}/ApplyTraining.jsp'; // Replace with your target page
					}, 1000);
				},
				error : function(xhr, status, error) {
		...
		function processData() {
			...
			$
					.ajax({
						type : "POST",
						url : "${pageContext.request.contextPath}/api/processData1234",
						data : {
							treasuryId : treasuryId,
							ref_planner_id : refPlannerId,
							venueid : venueId
