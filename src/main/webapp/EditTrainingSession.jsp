...
<body>
		...
		<form class="text-left" id="addSessionForm"
			...
			<div class="form-row">
				...
				<div style="display: none;">
									<label for="id">Session UID:</label> <input type="text" id="id"	readonly> 
						...
						<label for="sessionName">Session Name:</label>
							<input type="text" id="sessionName" name="sessionName"
							       ...
							       placeholder="Enter Year Range (e.g., 2023-2024)" 
							       required>
	...
	<script>
		...
		function apiCall(url, type, dataType, id, key, unique) {
		    $.ajax({
		        ...
		        success: function(data) {
		            ...
		            // Check if all items in sitems are present in unique
		...
		function fetchdata(endpoint, id, key, unique) {
		    apiCall("${pageContext.request.contextPath}"+ endpoint, "GET", "json", id, key, unique);
		}
			...
			$.ajax({
				...
				success : function(data) {
					...
					const status = $('#status');
	               	...
	               	unique.push(data.status)
	               	fetchdata("/api/session/get","status","status",unique);
