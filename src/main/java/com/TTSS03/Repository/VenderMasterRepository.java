...
public interface VenderMasterRepository extends JpaRepository<VenderMaster, Integer> {
	...
	@Query(value = "SELECT * FROM bdg_vendermaster where Record_Status ='Active'", nativeQuery = true)
	...
	@Query("SELECT c FROM VenderMaster c where c.vendorcode =(SELECT MAX(e.vendorcode) FROM VenderMaster e) ")
	Optional<VenderMaster> findEntityByHighestTcCode();
