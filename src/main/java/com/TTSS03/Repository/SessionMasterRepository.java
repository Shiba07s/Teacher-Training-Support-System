...
public interface SessionMasterRepository extends JpaRepository<SessionMaster, Long> {
	...
	@Query("SELECT t.sessionCode FROM SessionMaster t WHERE t.recordstatus = 'Active' AND t.status='Current' ")
	String findActiveAndCurrentSessionCode();
...
//    
	...
	@Modifying
	@Query("UPDATE SessionMaster a SET " + "a.sessionName = :sessionName, " + "a.sessionCode = :sessionCode, "
			+ "a.fromDate = :fromDate, " + "a.toDate = :toDate, " + "a.status = :status " + "WHERE a.id = :id")
	int updateSession(@Param("id") long id, @Param("sessionCode") String sessionCode,
			@Param("sessionName") String sessionName, @Param("fromDate") String fromDate,
			@Param("toDate") String toDate, @Param("status") String status);
...
//    int updateTrainingSpell(@Param("spellcode") long spellcode, @Param("spell") String spell);
	...
	Optional<SessionMaster> findByStatusCurrent();
}
