...
<body>
	<form id="addSessionForm" onsubmit="return saveGroup()">
		...
		<div class="container">
			<h2>Add Training Mode</h2>
			<div>
				<label for="trainingmode">Training Mode:</label> <input type="text"
					id="trainingmode" placeholder="Enter trainingmode" required>
	...
	<script>
		function validateForm() {
			...
			if (trainingMode.trim() === "") {
				document.getElementById("pname").innerHTML = "Training mode is required";
				return false;
			} else {
		...
		function addModeData() {
			if (validateForm()) {
				...
				$.ajax({
					type : "POST",
					url : "${pageContext.request.contextPath}/api/create/trainingMode",
					contentType : "application/json",
					data : JSON.stringify(formData),
					success : function(response) {
						console.log(response);
						toastr.success("Training mode saved successfully");
						setTimeout(function() {
