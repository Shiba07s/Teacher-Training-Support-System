...
<body>
	...
	<a href="${pageContext.request.contextPath}/AddNewComponentLimit.jsp" class="btn btn-success mt-2">Add Component Limit +</a>
	...
	<script>
		...
		function fetchDataAndDisplay() {
			$.ajax({
				type : "GET",
				url : "${pageContext.request.contextPath}/api/getallcomponentsbystatus",
				dataType : "json",
		...
		function displayTrainingData(data) {
			...
			for (var i = 0; i < trainings.length; i++) {
				...
				row.append("<td>" + training.budgeted_year + "</td>");
				row.append("<td>" + training.approved_rate + "</td>");
				row.append("<td >" + training.unit + "</td>");
				row.append("<td >" + training.record_status + "</td>");
				row
		...
		function deleteTraining(button,id) {
			...
			$.ajax({
				type : "Delete",
				url : "${pageContext.request.contextPath}/api/deletecomponent/" + id,
				...
				error : function(xhr, status, error) {
					toastr.error("Failed to delete Component: " + error);
				}
			});
			}
		}
		...
		function editTraining(sl) {
			...
			// Proceed with deletion only if the user confirms
			if (isConfirmed) {
			var editUrl = "${pageContext.request.contextPath}/EditComponentLimit.jsp?sl=" + sl;// Redirect to the EditTraining.jsp page with the query string parameters
