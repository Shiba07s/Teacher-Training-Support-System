...
<body>
	<form id="addSessionForm" onsubmit="return saveGroup()">
		...
		<div class="container">
			<h2>Add Training Grade</h2>

			<div>
				<label for="grade-name">Training Grade Name:</label> <input
					type="text" id="grade-name" placeholder="Enter grade name" required>
				...
				.ready(
						function() {
							function validateForm() {
								...
								var attendgrade = $("#grade-name").val();

								console.log(attendgrade);
								if (attendgrade.trim() === "") {
									document.getElementById("pname").innerHTML = "Training Grade is required";
									return false;
								} else {
							...
							window.saveGroup = function() {
								if (validateForm()) {
									...
									$
											.ajax({
												type : "POST",
												url : "${pageContext.request.contextPath}/api/grade/create",
												contentType : "application/json",
												data : JSON.stringify(formData),
												success : function(response) {
													console.log(response);
													toastr
															.success("Training Grade saved successfully");
													setTimeout(function(){
