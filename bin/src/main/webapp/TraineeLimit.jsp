...
<body>
	...
	<a href="${pageContext.request.contextPath}/AddNewTraineeLimit.jsp" class="btn btn-success mt-2">Add Trainees Limit +</a>
	...
	<script>
		...
		function displayTrainingData(data) {
			...
			for (var i = 0; i < trainings.length; i++) {
				...
				row.append("<td>" + training.expenseType + "</td>");
				row.append("<td>" + training.approvedRate + "</td>");
		...
		function deleteTraining(button,id) {
			...
			$.ajax({
				type : "Delete",
				url : "${pageContext.request.contextPath}/api/deleteTraineesLimit/" + id,
				success : function(response) {
					var table = $('#trainingTable').DataTable();
		            table.row(row).remove().draw(false);
					toastr.success("Trainee limit deleted successfully");
					// Reload or update the data after deletion
					fetchDataAndDisplay();
				},
				error : function(xhr, status, error) {
					toastr.error("Failed to delete Trainee limit: " + error);
				}
			});
			}
		}
		...
		function editTraining(sl) {
			...
			// Proceed with deletion only if the user confirms
			if (isConfirmed) {
			var editUrl = "${pageContext.request.contextPath}/EditTraineeLimit.jsp?sl=" + sl;// Redirect to the EditTraining.jsp page with the query string parameters
